...
void securedic::slot_SecuredIC_Start()
{
    ...
    if(sysParm->mRfDeviceFactors->SIC_result == false)
    {
     ...
     OWOverdriveSkipROM();
    /////////////////////////////////First DS28E15////////////////////////////
    qDebug("First Device Check & Read");
    rt = Find_First_DS28E15(&device_num_pages);
...
#if 0
    /////////////////////////////////Next DS28E15////////////////////////////
    rt = Find_Next_DS28E15(&device_num_pages);
    // read personality bytes to get manufacturer ID
    if (ReadStatus(TRUE, FALSE, 0, buf))
    {
       manid[0] = buf[3];
...
//
int securedic::Find_First_DS28E15(int *num_pages)
{
   int i, rt = TRUE;

   dprintf("Find the connected First DS28E15\n");
   // find first device on 1-Wire network
   if (OWFirst())
   {
      if ((ROM_NO[0] & 0x7F) == DS28E15_FAMILY)
      {
         dprintf("DS28E15 First Found: ");
         for (i = 0; i < 8; i++)
...
int securedic::Find_Next_DS28E15(int *num_pages)
{
   int i, rt = TRUE;

   dprintf("Find the connected Next DS28E15\n");
   // find first device on 1-Wire network
   if (OWNext())
   {
      if ((ROM_NO[0] & 0x7F) == DS28E15_FAMILY)
      {
         dprintf("DS28E15 Next Found: ");
         for (i = 0; i < 8; i++)
            dprintf("%02X ", ROM_NO[i]);
         dprintf("\n");
         *num_pages = DS28E15_PAGES;
