...
void sysmessage::SetFont()
{
    ...
    QFont title_font(family,25, QFont::Bold);
    QFont data_font(family,20, QFont::Bold);
    ...
    ui->Message_title_label->setAlignment(Qt::AlignCenter);
    ui->Message_data_label->setAlignment(Qt::AlignCenter);

    //ui->Action_label->setFont(label_font);
    //ui->Action_No_label->setFont(Action_label_font);
}

void sysmessage::slot_alert_state(Alert_State state)
{
    ui->screen_label->setPixmap(QPixmap(":/Images/alarm.png"));
    ui->screen_label->setScaledContents(true);
    switch(state)
    {
        case ALARM_ACTIVE_ANTENNA_NOT_CONNECT:
            ui->Message_title_label->setText("ALERT : ");
            ui->Message_data_label->setText("Antenna Not Connect");

            break;
        case ALARM_ACTIVE_ANTENNA_OVER_TEMPERATURE:
            ui->Message_title_label->setText("ALERT : ");
            ui->Message_data_label->setText("Antenna Over Temperature");
...
void sysmessage::slot_error_state(Error_State state)
{
    ui->screen_label->setPixmap(QPixmap(":/Images/error.png"));
    ui->screen_label->setScaledContents(true);
    switch(state)
    {
        case ERROR_READY_ACTIVE_AMP_OVER_TEMPERATURE:
            ui->Message_title_label->setText("ERROR : ");
            ui->Message_data_label->setText("Amp Over Temperature");

            break;
        case ERROR_STANDBY_ANTENNA_NOT_IDENTIFIED:
            ui->Message_title_label->setText("ERROR : ");
            ui->Message_data_label->setText("Antenna Not Connect");

            break;
        case ERROR_STANDBY_READY_ANTENNA_OVERLAP_CHECK:
            ui->Message_title_label->setText("ERROR : ");
            ui->Message_data_label->setText("Antenna Overlap Check");
